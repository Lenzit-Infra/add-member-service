...
def select_best_agent(db: Session, daily_limit: int) -> Optional[Agent]:
    """The actual anti-ban assignment rule: among active, non-banned,
    not-in-flood-cooldown, under-daily-limit agents, pick the one used the
    LEAST today. This spreads volume across the whole pool instead of
    draining one account first — keeping any single account's daily add
    volume low is the single biggest lever against Telegram flagging it."""
    today_counts = get_today_counts(db)
    eligible = [a for a in _eligible_candidates(db) if today_counts.get(a.id, 0) < daily_limit]
    if not eligible:
        return None
...
def count_eligible_agents(db: Session, daily_limit: int) -> int:
    today_counts = get_today_counts(db)
    return sum(1 for a in _eligible_candidates(db) if today_counts.get(a.id, 0) < daily_limit)


def agent_status_info(db: Session, agent: Agent, daily_limit: int, today_counts: Optional[Dict[int, int]] = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "needs_review": get_recent_failure_ratio(db, agent.id) >= 0.7,
    }
